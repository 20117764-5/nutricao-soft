...
import { 
  ...
  Calendar as CalendarIcon, Plus, Trash2, Send, FileDown, ChevronDown, ChevronUp
} from 'lucide-react';
...
export default function PerfilPacientePage() {
  ...
  const fetchDataAbas = useCallback(async () => {
    ...
    if (activeTab === 'anamnese') {
      try {
        const qTemplates = supabase.from('anamneses_templates').select('*').eq('paciente_id', id).order('created_at', { ascending: false });
        const qRespostas = supabase.from('anamneses_respostas').select('*').eq('paciente_id', id);
        
        // Busca inteligente no JSONB pelo CPF
        const qForms = paciente.cpf 
          ? supabase.from('form_responses').select('id, form_title, created_at, responses').contains('patient_data', { cpf: paciente.cpf })
          : null;

        const [resT, resR, resF] = await Promise.all([
          qTemplates,
          qRespostas,
          qForms || Promise.resolve({ data: [] })
        ]);

        setTemplatesAtivos(resT.data as AnamneseTemplate[] || []);

        const aMapeadas: RespostaUnificada[] = (resR.data as DbAnamneseRes[] || []).map(r => ({
          id: r.id,
          titulo: r.titulo || 'Anamnese Respondida',
          data: r.respondido_em,
          tipo: 'anamnese' as const,
          conteudo: r.respostas
        }));

        const pMapeadas: RespostaUnificada[] = (resF.data as DbFormResponse[] || []).map(p => ({
          id: p.id,
          titulo: p.form_title || 'Formulário Externo',
          data: p.created_at,
          tipo: 'pre_consulta' as const,
          conteudo: p.responses
        }));

        const unificado = [...aMapeadas, ...pMapeadas].sort(
          (a, b) => new Date(b.data).getTime() - new Date(a.data).getTime()
        );

        setHistoricoAnamneses(unificado);
      } catch (err) {
  ...
  // --- HANDLERS ---
  const handleOpenNewAnamnese = () => {
    setEditingTemplateId(null);
    setAnamneseTitulo('');
    setAnamnesePerguntas(['']);
  ...
  const handleDeleteTemplate = async (templateId: string) => {
    if (!confirm('Excluir este link de anamnese?')) return;
    const { error } = await supabase.from('anamneses_templates').delete().eq('id', templateId);
    if (!error) fetchDataAbas();
  };
  ...
  const handleGerarAnamnese = async () => {
    if (!anamneseTitulo || anamnesePerguntas.some(p => !p.trim())) return alert('Preencha os campos.');
    setSavingAnamnese(true);
    try {
      const payload = { paciente_id: id, titulo: anamneseTitulo, perguntas: anamnesePerguntas };
      if (editingTemplateId) {
        await supabase.from('anamneses_templates').update(payload).eq('id', editingTemplateId);
      } else {
        await supabase.from('anamneses_templates').insert(payload);
      }
      setIsAnamneseModalOpen(false);
      fetchDataAbas();
    } catch (err) { alert('Erro ao salvar.'); } finally { setSavingAnamnese(false); }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      fetchPaciente();
    } catch (err) { alert('Erro ao atualizar.'); } finally { setSaving(false); }
  };

  const exportarParaPDF = (item: RespostaUnificada) => {
    if (!paciente) return;
    const doc = new jsPDF();
    let y = 20;
    doc.setFontSize(16);
    ...
    doc.setFontSize(10);
    doc.setTextColor(40);
    doc.text(`Paciente: ${paciente.nome}`, 20, y);
    y += 10;
    Object.entries(item.conteudo).forEach(([p, r]) => {
      if (y > 270) { doc.addPage(); y = 20; }
      doc.setFont('helvetica', 'bold');
      doc.text(doc.splitTextToSize(p.toUpperCase(), 170), 20, y);
      y += 7;
      doc.setFont('helvetica', 'normal');
      doc.text(doc.splitTextToSize(r || '—', 170), 20, y);
      y += 12;
    });
    doc.save(`${item.titulo}.pdf`);
  };
    ...
    <div className="fixed inset-0 z-[100] bg-gray-50 flex overflow-hidden animate-in fade-in duration-300">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col overflow-y-auto shrink-0">
        <div className="p-6 border-b border-gray-100">
          <div className="w-12 h-12 bg-nutri-primary/10 rounded-full flex items-center justify-center text-nutri-primary font-bold text-xl mb-3 uppercase">{paciente.nome.charAt(0)}</div>
          <h2 className="font-bold text-gray-800 truncate">{paciente.nome}</h2>
          <p className="text-xs text-gray-500 font-medium tracking-wide">Paciente Ativo</p>
        </div>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-8 py-5 flex justify-between items-center shadow-sm">
          <h1 className="text-xl font-bold text-gray-800">
            {paciente.nome}, <span className="text-gray-500 font-normal">
              {paciente.data_nascimento ? differenceInYears(new Date(), parseISO(paciente.data_nascimento)) : '--'} anos
            </span>
          </h1>
          <button onClick={() => router.push('/admin/consultorio/pacientes')} className="p-2 hover:bg-red-50 rounded-full text-gray-400 hover:text-red-500 transition-colors"><X size={24} /></button>
        </header>
          ...
          <div className="max-w-5xl mx-auto">
              ...
              <div className="space-y-8 animate-in slide-in-from-bottom-2 duration-300">
                <section className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                  <div className="flex justify-between items-start mb-6">
                    <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2"><User className="w-5 h-5 text-nutri-primary" /> Dados Pessoais</h3>
                    <button onClick={() => setIsEditModalOpen(true)} className="text-sm font-bold text-nutri-primary flex items-center gap-1 hover:bg-nutri-50 px-3 py-1.5 rounded-lg transition-colors"><Edit className="w-4 h-4" /> Editar</button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <InfoField label="Nome Completo" value={paciente.nome} />
                    <InfoField label="Data de Nascimento" value={paciente.data_nascimento ? format(parseISO(paciente.data_nascimento), 'dd/MM/yyyy') : '-'} />
                    <InfoField label="Telefone" value={paciente.telefone || '-'} icon={<Phone className="w-3 h-3 text-gray-400 mr-1"/>}/>
                    <InfoField label="CPF" value={paciente.cpf || '-'} />
                  </div>
                </section>

                {/* FLUXO DE CONSULTA COM ÍCONES BRANCO OPACO */}
                <section>
                  <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2"><Activity className="w-5 h-5 text-nutri-primary" /> Fluxo de Consulta</h3>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    <ActionCard icon={<CalendarPlus size={24}/>} label="Agendar" color="bg-blue-500" onClick={() => {}} />
                    <ActionCard icon={<CheckCircle2 size={24}/>} label="Consulta" color="bg-green-600" onClick={() => {}} />
                    ...
                    <ActionCard icon={<Utensils size={24}/>} label="Planejamento" color="bg-orange-500" onClick={() => setActiveTab('planejamento')} />
            ...
            {activeTab === 'historico' && (
              <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm animate-in slide-in-from-bottom-2">
                <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2"><ClipboardList className="w-5 h-5 text-nutri-primary" /> Histórico</h3>
                <div className="relative border-l-2 border-gray-100 ml-4 space-y-8 pb-4">
                  {consultas.map((c) => (
                    <div key={c.id} className="relative pl-8">
                      <div className={`absolute left-[-9px] top-0 w-4 h-4 rounded-full border-2 border-white ${c.status === 'Realizada' ? 'bg-green-500' : 'bg-blue-500'}`}></div>
                      <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-sm font-bold text-gray-700">{format(parseISO(c.data_consulta), "dd/MM/yyyy")}</span>
                          <span className="text-[10px] font-black uppercase text-gray-400">{c.status}</span>
                        </div>
            ...
            {activeTab === 'anamnese' && (
              <div className="space-y-8 animate-in slide-in-from-bottom-2">
                <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                  ...
                  <button onClick={handleOpenNewAnamnese} className="bg-nutri-dark text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg hover:bg-black transition-all">
                ...
                {/* HISTÓRICO DE RESPOSTAS COM ACCORDION */}
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2 px-2"><CheckCircle2 size={14}/> Histórico de Respostas</h4>
                  {historicoAnamneses.length === 0 ? (
                    <div className="bg-white p-10 rounded-2xl border border-dashed text-center text-gray-400 italic">Nenhuma resposta encontrada.</div>
                  ) : (
                      ...
                      <div key={item.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden group hover:border-nutri-primary transition-all">
                        <div className="p-6 flex justify-between items-center cursor-pointer bg-white" onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}>
                          <div className="flex items-center gap-4">
                            <div className={`p-2 rounded-lg ${item.tipo === 'anamnese' ? 'bg-purple-50 text-purple-600' : 'bg-blue-50 text-blue-600'}`}>
                              <FileText size={20}/>
                            </div>
                            <div>
                              <h4 className="font-bold text-gray-800">{item.titulo}</h4>
                              <p className="text-[10px] uppercase font-black text-gray-300">
                                {item.tipo === 'anamnese' ? 'Anamnese Dinâmica' : 'Pré-Consulta'} • {format(parseISO(item.data), 'dd/MM/yyyy')}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                            <button onClick={() => exportarParaPDF(item)} className="p-2 text-gray-400 hover:text-nutri-primary transition-all"><FileDown size={22}/></button>
                            <button onClick={() => handleDeleteResposta(item)} className="p-2 text-gray-400 hover:text-red-500 transition-all"><Trash2 size={20}/></button>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                              {Object.entries(item.conteudo).map(([p, r]) => (
                                <div key={p} className="space-y-1">
                                  <p className="text-[10px] font-black text-gray-400 uppercase leading-relaxed">{p}</p>
                                  <p className="text-sm text-gray-700 font-medium bg-white p-3 rounded-lg border border-gray-200">{r || '—'}</p>
                                </div>
                              ))}
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 z-[200] bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
            <div className="p-6 border-b flex justify-between items-center bg-gray-50 rounded-t-2xl">
              <h2 className="text-xl font-bold text-gray-800">{editingTemplateId ? 'Editar' : 'Nova'} Anamnese</h2>
              <button onClick={() => setIsAnamneseModalOpen(false)}><X size={24}/></button>
            </div>
            <div className="p-8 overflow-y-auto space-y-6">
              <input type="text" placeholder="Título" className="w-full border p-3 rounded-xl outline-none font-bold text-lg" value={anamneseTitulo} onChange={e => setAnamneseTitulo(e.target.value)} />
              {anamnesePerguntas.map((p, i) => (
                <div key={i} className="flex gap-3">
                  <input type="text" className="flex-1 border p-3 rounded-xl bg-gray-50 outline-none" value={p} onChange={(e) => {
                    const n = [...anamnesePerguntas]; n[i] = e.target.value; setAnamnesePerguntas(n);
                  }} />
                  <button onClick={() => setAnamnesePerguntas(anamnesePerguntas.filter((_, idx) => idx !== i))} className="p-2 text-gray-300 hover:text-red-500 transition-colors"><Trash2 size={20} /></button>
                </div>
              ))}
              <button onClick={() => setAnamnesePerguntas([...anamnesePerguntas, ''])} className="w-full py-4 border-2 border-dashed rounded-xl text-gray-400 font-bold hover:text-nutri-primary transition-all flex items-center justify-center gap-2"><Plus size={20} /> Adicionar Pergunta</button>
            </div>
            <div className="p-6 border-t bg-gray-50 rounded-b-2xl flex justify-end gap-3">
               <button onClick={() => setIsAnamneseModalOpen(false)} className="px-6 py-2.5 font-bold text-gray-500">Cancelar</button>
               <button onClick={handleGerarAnamnese} disabled={savingAnamnese} className="bg-nutri-dark text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-all hover:bg-black">
                 {savingAnamnese ? <Loader2 className="animate-spin" /> : <><Save size={18} /> Salvar Anamnese</>}
               </button>
            </div>
          </div>
        </div>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-[200] bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl p-8 animate-in zoom-in duration-200">
            <div className="flex justify-between items-center border-b pb-4 mb-6"><h2 className="text-xl font-bold text-gray-800">Editar Dados</h2><button onClick={() => setIsEditModalOpen(false)}><X size={24} className="text-gray-400"/></button></div>
            <div className="space-y-4">
              ...
              <button onClick={handleUpdate} disabled={saving} className="w-full bg-nutri-dark text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 mt-4 hover:bg-black transition-all shadow-lg">{saving ? <Loader2 className="animate-spin" /> : <><Save size={18}/> Salvar Alterações</>}</button>
            </div>
          </div>
        </div>
      )}
...
function InfoField({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <label className="block text-[10px] font-black text-gray-400 uppercase mb-1 tracking-widest">{label}</label>
      <div className="flex items-center font-bold text-gray-800 text-sm border-b border-gray-100 pb-1">{icon}{value}</div>
    </div>
  );
}

// COMPONENTE ATUALIZADO COM ÍCONES BRANCOS OPACOS
function ActionCard({ icon, label, color, onClick }: { icon: React.ReactNode; label: string; color: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex flex-col items-center justify-center p-4 rounded-2xl shadow-sm hover:scale-105 transition-all h-32 w-full group ${color}`}>
      <div className="bg-white/20 p-3 rounded-full mb-3 group-hover:bg-white/30 transition-all text-white/80">
        {icon}
      </div>
      <span className="text-white font-bold text-sm text-center leading-tight">{label}</span>
    </button>
  );
}